...
// QueryWithin queries the within edge of a User.
func (c *UserClient) QueryWithin(u *User) *GroupQuery {
	query := (&GroupClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
// QueryDirectRoles queries the direct_roles edge of a User.
func (c *UserClient) QueryDirectRoles(u *User) *RoleQuery {
	query := (&RoleClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Group, Role, User []ent.Hook
	}
	inters struct {
		Group, Role, User []ent.Interceptor
	}
)
